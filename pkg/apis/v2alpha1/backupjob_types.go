...
// BackupJob is the schema for the backup job API.
...
type BackupSpec struct {
	// FIXME: should probably drop the 'Cluster' suffix for consistency with other resources and is 'source' the right term?
	// SourceClusterName is the reference to the source cluster.
...
type BackupTemplate struct {
	// BackupToolName is the backup tool to use for the backup.
	// +kubebuilder:validation:Required
	...
	// These options are specific to the BackupTool being used and must conform to
	// the schema defined in the BackupTool's .spec.config.openAPIV3Schema.
...
type BackupDestination struct {
	...
	// FIXME
	// Path is the path to the directory to store the backup.
	// This may be a path to a file or a directory, depending on the backup tool.
...
// BackupJobState is a type representing the state of a backup job.
type BackupJobState string

const (
	// BackupJobStatePending indicates that the backup job is pending.
	BackupJobStatePending BackupJobState = "Pending"
	// BackupJobStateRunning indicates that the backup job is currently running.
	BackupJobStateRunning BackupJobState = "Running"
	// BackupJobStateSucceeded indicates that the backup job has completed successfully.
	BackupJobStateSucceeded BackupJobState = "Succeeded"
	// BackupJobStateFailed indicates that the backup job has failed.
	// Once the job is in this phase, it cannot be retried.
	BackupJobStateFailed BackupJobState = "Failed"
	// BackupJobStateError indicates that the backup job has encountered an error.
	// This phase is used for transient errors that may allow the job to be retried.
	BackupJobStateError BackupJobState = "Error"
)
...
type BackupJobStatus struct {
	...
	State BackupJobState `json:"state,omitempty"`
