...
// BackupTool defines a reusable strategy for backing up data from a DataStore.
...
// BackupToolSpec defines the specification of a BackupTool.
type BackupToolSpec struct {
	...
	Config BackupToolConfig `json:"config,omitempty"`
	...
	// +optional
	DataStoreConstraints BackupToolDataStoreConstraints `json:"dataStoreConstraints,omitempty"`
...
// BackupToolConfig contains additional configuration defined for the backup tool.
...
// ErrSchemaValidationFailure is returned when the parameters do not conform to the BackupTool schema defined in .spec.config.
var ErrSchemaValidationFailure = errors.New("schema validation failed")

// Validate the config for the backup tool.
func (cfg *BackupToolConfig) Validate(params *runtime.RawExtension) error {
...
// BackupToolDataStoreConstraints defines compatibility requirements and prerequisites
// that must be satisfied by a DataStore before this backup tool can be used with it.
type BackupToolDataStoreConstraints struct {
...
// BackupToolStatus defines the status of the BackupTool.
